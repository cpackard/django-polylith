...
from demo_namespace.choices import interface as choices
from demo_namespace.questions import interface as questions
...
def questions_view(request: HttpRequest) -> JsonResponse:
    ...
    if question is None:
        return JsonResponse(
            {"errors": ['You must include the "question" parameter.']}, status=400
        )
...
def choices_view(request: HttpRequest) -> JsonResponse:
    ...
    if request.method == "GET":
        ...
        if choice is None:
            return JsonResponse(
                {"errors": ['You must include the "choice" parameter.']}, status=400
            )

        return JsonResponse(
            {"result": choices.create_choice(question, choice)}, status=201
        )
